...
<body style="background-color: #022130; font-family: Segoe UI Light; color: white;">
    ...
    <form id="form1" runat="server" style="text-align: center;">
      ...
      ,[SR_REPORTDATE]
      ,[REPORTDATE]     
  FROM [ODW].[EAMS].[FactWorkOrder]
  WHERE asset_key = (select ASSET_KEY from EAMS.DimAsset where ASSETNUM = @ID)" >
                  <SelectParameters>
                    <asp:QueryStringParameter DbType="String" DefaultValue="10010857" Name="ID" QueryStringField="ID" />
                </SelectParameters>
            </asp:SqlDataSource>
            <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:ODWConnectionString %>"
                SelectCommand="SELECT Count(*) AS TotalRows FROM [ODW].[GIS].[vwWorkOrder] w where w.asset_key = (select ASSET_KEY from EAMS.DimAsset a where a.ASSETNUM = @ID)">
                           <SelectParameters>
                    <asp:QueryStringParameter DbType="String" DefaultValue="10010857" Name="ID" QueryStringField="ID" />
                </SelectParameters>
