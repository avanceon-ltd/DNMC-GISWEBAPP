    ...
    <form id="form1" runat="server" style="text-align: center;">
        <div>
            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:ODWConnectionString %>"
                SelectCommand="SELECT TOP 1000  [ASSETLOCPRIORITY]
      ,[CHANGEBY]
      ,[CHANGEDATE]    
      ,[DESCRIPTION]
      ,[EXT_ASSETTYPE]  
      ...
      ,[ACTSTART]    
      ,[SR_REPORTDATE]
      ,[REPORTDATE]     FROM [ODW].[EAMS].[FactWorkOrder]  where [LOCATION_KEY] = 
(SELECT [LOCATION_KEY] FROM [ODW].[EAMS].[DimLocation] where [LOCATION]= @ID) order by CHANGEDATE desc" >
                  <SelectParameters>
                    <asp:QueryStringParameter DbType="String" DefaultValue="QATAR" Name="ID" QueryStringField="ID" />
                </SelectParameters>
            </asp:SqlDataSource>
            <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:ODWConnectionString %>"
                SelectCommand="SELECT  Count(*) AS TotalRows FROM [ODW].[EAMS].[FactWorkOrder]  where [LOCATION_KEY] = (SELECT [LOCATION_KEY] FROM [ODW].[EAMS].[DimLocation] where [LOCATION]= @ID)">
                           <SelectParameters>
                    <asp:QueryStringParameter DbType="String" DefaultValue="QATAR" Name="ID" QueryStringField="ID" />
                </SelectParameters>
