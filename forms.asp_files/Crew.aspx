    ...
    <form id="form1" runat="server" style="text-align: center;">
        <div>
            ...
            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:ODWConnectionString %>" 
                SelectCommand="select PERSON_KEY,PERSONID,EXT_RESPSECTION,EXT_UNIT,EXT_RESPAREA,[SUPERVISOR] from [EAMS].[DimPerson] where ([STATUS] = 'Active' AND COMPANY_KEY <> -1 and COMPANY_KEY = (Select  Company_Key From EAMS.DimCompany where COMPANY = @Company) and personid in (select respparty from [EAMS].[DimPersonGroup] where persongroup ='CREWLEAD'))"
            ...
            <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:ODWConnectionString %>" 
                SelectCommand="select count(*) As TotalRows from [EAMS].[DimPerson] where ([STATUS] = 'Active' AND COMPANY_KEY <> -1 and COMPANY_KEY = (Select  Company_Key From EAMS.DimCompany where COMPANY = @Company) and personid in (select respparty from [EAMS].[DimPersonGroup] where persongroup ='CREWLEAD'))">
                <SelectParameters>
                    <asp:QueryStringParameter DbType="String" Name="Company" QueryStringField="Company" />
                </SelectParameters>
            </asp:SqlDataSource>
