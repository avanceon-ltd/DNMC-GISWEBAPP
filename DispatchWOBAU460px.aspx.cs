...
namespace WebAppForm
{
    ...
    public partial class DispatchWOBAU460px : System.Web.UI.Page
    {
            ...
            if (!Page.IsPostBack || Reloadbtnclicked)
            {
                ...
                if (!String.IsNullOrEmpty(Request.QueryString["WONUM"]))
                {
                    ...
                    if (!String.IsNullOrEmpty(Request.QueryString["backlink"]))
                    {
                        ...
                        lnkbtnback.PostBackUrl = "WorkOrderDetail.aspx?ID=" + lblWONUM.Text.Trim() + "&CRMSID=" + Request.QueryString["CRMSID"] + "&USER=" + Request.QueryString["USER"];
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                if (ViewState["DispatchWOBAU460Obj"] != null)
                {
                    objWorkOrder = (WorkOrder)ViewState["DispatchWOBAU460Obj"];
                }
                    ...
                    FROM [ODW].[EAMS].vwFactWorkOrder w WHERE WONUM = @WONUM", con))
                {
                    ...
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            ...
                            ViewState["DispatchWOBAU460Obj"] = objWorkOrder;
