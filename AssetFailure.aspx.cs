    ...
    public partial class AssetFailure : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                {
                    btnSubmit.Text = "Indicate Asset Failure";
                    txtAssetDes.ReadOnly = false;
                    txtlong.ReadOnly = false;
                    txtLat.ReadOnly = false;
                    btnDel.Visible = false;
                    btnDel.Enabled = false;
                    lbl_label.Text = "Indicate Asset Failure - Express Form";
                ...
                using (SqlCommand cmd = new SqlCommand(@"select * from [ODW].[GIS].[AssetFailure] where UID=@UID", con))
                {
                    ...
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            ...
                            userName.Text = reader["loggedinUser"].ToString();
                        }
                    }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["ODWConnectionString"].ConnectionString))
                {
                    ...
                    {
                        using (SqlCommand cmd = new SqlCommand(@"UPDATE [GIS].[AssetFailure] SET [Status]=@Status ,[Remarks]=@Remarks,[ModifiedDate]=@ModifiedDate,loggedinUser=@loggedinUser  WHERE UID=@UID", con))
                        {
                    ...
                    {
                        using (SqlCommand cmd = new SqlCommand(@"insert into [ODW].[GIS].[AssetFailure] ([Description],[Lat],[Long],[Shape],[Status],[Remarks],[loggedinUser]) values(@AD, @lat, @long,[geography]::STGeomFromText(((('POINT('+CONVERT([varchar](50),@long,(0)))+' ')+CONVERT([varchar](50),@lat,(0)))+')',(4326)),@Status,@Remarks,@loggedinUser)", con))
                        {
