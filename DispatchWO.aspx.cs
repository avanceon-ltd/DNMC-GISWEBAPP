...
namespace WebAppForm
{
    ...
    public partial class DispatchWO : System.Web.UI.Page
    {
        ...
        private void FillWODetail(string wonum)
        {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["ODWConnectionString"].ConnectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(@"SELECT LONGITUDEX,LATITUDEY,OWNER,WOPRIORITY,VENDOR,ONBEHALFOF,EXT_SCHEDULER,EXT_ENGINEER,EXT_INSPECTOR,
                    ...
                    (Select ALNVALUE from EAMS.FRWorkOrderSpecification (nolock) where WONUM=@WONUM and ASSETATTRID = 'HOTSPOT NUMBER') HOTSPOT,
					(Select ALNVALUE from EAMS.FRWorkOrderSpecification (nolock) where WONUM=@WONUM and ASSETATTRID = 'PROJECT ID') PROJECT,  
        ...
        private string GetHotspotID(string wonum)
        {
            ...
            "h.Shape.STDistance(geography::STGeomFromText((SELECT Shape FROM EAMS.FRFactWorkOrder WHERE WONUM ='" + wonum + "').ToString(), 4326)) AS Distance" +
            " FROM GIS.vwHotSpot h " +
            " WHERE h.Shape.STBuffer(" + Convert.ToInt32(50) + ").STContains(geography::STGeomFromText((SELECT Shape FROM EAMS.FRFactWorkOrder WHERE WONUM ='" + wonum + "').ToString(), 4326)) = 1 " +
